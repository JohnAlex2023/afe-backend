...
│                      LAYER API (FastAPI)                     │
│  ┌────────────────────────────────────────────────────────┐ │
│  │  routers/auth.py      → Autenticación (JWT + OAuth2)  │ │
│  │  routers/facturas.py  → CRUD facturas con paginación  │ │
│  │  routers/workflow.py  → Aprobación/rechazo            │ │
│  │  routers/automation.py→ Procesamiento automático       │ │
│  │  routers/*.py         → 11 routers más                 │ │
│  └────────────────────────────────────────────────────────┘ │
│                           │
│  Middleware: CORS, Auth, Exception Handlers
└──────────────────────────┬──────────────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────────────┐
│              BUSINESS LOGIC LAYER (Services)                 │
│  ┌────────────────────────────────────────────────────────┐ │
│  │ • workflow_automatico.py     → Automatización          │ │
│  │ • automation_service.py      → Procesamiento           │ │
│  │ • email_service.py           → SMTP + Graph API       │ │
│  │ • invoice_service.py         → Procesamiento facturas  │ │
│  │ • comparador_items.py        → Matching inteligente    │ │
│  │ • item_normalizer.py         → Normalización items     │ │
│  │ • audit_service.py           → Auditoría              │ │
│  │ • 20+ servicios más                                    │ │
│  └────────────────────────────────────────────────────────┘ │
│                           │
│  CRUD Layer (CRUD abstraction)
└──────────────────────────┬──────────────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────────────┐
│            DATA ACCESS LAYER (SQLAlchemy ORM)                │
│  ┌────────────────────────────────────────────────────────┐ │
│  │ Models: Responsable, Factura, Proveedor, Role, etc.   │ │
│  │ Session Management: Pool conexiones, transactions      │ │
│  └────────────────────────────────────────────────────────┘ │
└──────────────────────────┬──────────────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────────────┐
...
│  │ • roles                                                │ │
│  │ • facturas (principal, 10k+ registros)               │ │
│  │ • factura_items (líneas)                             │ │
│  │ • proveedores                                         │ │
...
EXTERNOS:
┌─────────────────────────────────────────────────────────────┐
│ • Microsoft Azure AD (OAuth, Microsoft Graph API)            │
│ • SMTP Server (Gmail, corporativo, etc.)                     │
│ • Scheduler (APScheduler para tareas background)             │
└─────────────────────────────────────────────────────────────┘
```
...
❌ Evitar:
   - RouterUsuarios.py (camelCase)
   - Servicios/ (mayúscula inicial)
```
...
app/
├── handlers.py          # Todo mezclado: auth, facturas, etc.
├── business.py          # Servicios sin separación clara
└── database.py          # Modelos sin organización
```
...
# ❌ EVITAR
def obtener_facturas(usuario_id, estado=None, limit=10):
    pass
```
...
|-----|----------|----------------|
| **Tech Lead Backend** | Equipo Backend | Lun-Vie 8:00-18:00 |
| **DevOps** | DevOps Team | 24/7 (on-call) |
| **Soporte** | support@empresa.com | Lun-Vie 9:00-17:00 |
...
- `API_AUTHENTICATION.md` - Detalles de autenticación

### Herramientas Recomendadas

- **Postman**: Testing de APIs
- **DBeaver**: Cliente gráfico MySQL
- **VS Code Extensions**: FastAPI, SQLAlchemy, Pylance
